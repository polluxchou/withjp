...
const DEPT_META: Record<string, { color: string; initials: string }> = {
  bd:      { color: 'bg-blue-500',    initials: 'BD' },
  ops:     { color: 'bg-purple-500',  initials: 'OP' },
  finance: { color: 'bg-emerald-500', initials: 'FI' },
  content: { color: 'bg-pink-500',    initials: 'CO' },
  growth:  { color: 'bg-orange-500',  initials: 'GR' },
  legal:   { color: 'bg-slate-600',   initials: 'LG' },
}
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-slate-400 truncate">{roleLabel(agent.role)}</div>
                  </div>
                  {!agent.is_active && (
                    <span className="ml-auto text-xs text-slate-300">{t('offline')}</span>
                  )}
                </button>
              )
            })
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {selectedAgent ? (
          ...
          <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-200 bg-white">
            ...
            <span className="text-sm text-slate-400">{t('selectAgent')}</span>
          </div>
        )}
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {Object.entries(DEPT_META).map(([role, { color, initials }]) => (
                  <div key={role} className="bg-white border border-slate-200 rounded-xl p-4 text-center">
                    ...
                    <div className="text-xs font-medium text-slate-700">{roleLabel(role)}</div>
                  </div>
                ))}
              </div>
              <p className="text-sm text-slate-400">{t('chooseAgent')}</p>
            </div>
          )}
        ...
        <div className="flex-shrink-0 px-6 pb-6 pt-2">
          <form onSubmit={sendMessage} className="flex items-end gap-3 bg-white border border-slate-200 rounded-2xl px-4 py-3 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-300 transition-all">
            <textarea
              ...
              placeholder={selectedAgent ? t('messagePlaceholder', { agent: selectedAgent.name.split(' ')[0] }) : t('selectAgentFirst')}
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="max-w-[72%]">
        ...
        <div className="text-xs text-slate-400 mt-1 ml-1">
          {agent?.name.split(' ')[0] ?? t('agentFallback')}
        </div>
      </div>
    </div>
  )
