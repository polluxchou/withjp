...
export default function TimelineClient({ initialMilestones }: { initialMilestones: Milestone[] }) {
  ...
  // Data arrives server-rendered — no mount fetch, no first-paint spinner.
  // status/type are stored columns, so filtering in memory matches what the
  // server query returned; router.refresh() re-runs the server component to
  // pull fresh data after create/delete.
  const milestones = initialMilestones.filter(m =>
  ...
  return (
    <div>
      ...
      {milestones.length === 0 ? (
        ...
        <ListView milestones={milestones} onUpdated={refresh} />
      ) : view === 'gantt' ? (
