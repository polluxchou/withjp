...
export default function CreatorsClient({ initialCreators, loadError }: Props) {
  ...
  // Data arrives server-rendered via props — no client fetch, no first-paint
  // spinner. Both status filter and search run in-memory, so switching tabs is
  // instant. router.refresh() re-runs the server component to pull fresh data
  // (e.g. after adding a creator).
  const filtered = initialCreators.filter((c) => {
    ...
    return !q ||
      ...
      (c.broadcast_account?.name ?? '').toLowerCase().includes(q) ||
      (c.operator_user?.name ?? '').toLowerCase().includes(q) ||
      (c.operator_user?.user_code ?? '').toLowerCase().includes(q)
  })
  ...
  return (
    <div>
      <Header
        title={t('title')}
        subtitle={t('subtitle', { count: initialCreators.length })}
        actions={
      ...
      {showForm && (
        <Modal open={showForm} onClose={() => setShowForm(false)} title={t('addCreator')} width="max-w-2xl">
          <CreatorForm onSuccess={() => { setShowForm(false); router.refresh() }} onCancel={() => setShowForm(false)} />
        </Modal>
      )}
    </div>
  )
