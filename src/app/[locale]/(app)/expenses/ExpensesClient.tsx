...
type Props = {
  // Server-rendered seeds so the page paints with data on first load instead of
  // a spinner. The client keeps all its existing fetch/filter machinery; these
  // only prime the initial state (and match what the first client fetch returns
  // for the same URL, so the background refresh is a no-op visually).
  initialExpenses: Expense[]
  initialError: string | null
  initialMonths: string[]
}

export default function ExpensesClient({ initialExpenses, initialError, initialMonths }: Props) {
  ...
  // Seeded data counts as the first load, so the mount refetch runs silently
  // (no full-page skeleton) instead of blanking the server-rendered content.
  ...
  const [availableMonths, setAvailableMonths] = useState<Set<string>>(() => new Set(initialMonths))
