...
export default function FinanceForecastDashboard({
  ...
  /** Add an account by cloning the last row in the current month as a
   *  template — copies type / hours / minute-revenue / share ratio but
   *  resets the name. Falls back to addRow() when no rows exist yet. */
  function addRowFromTemplate() {
    const current = months[safeSelectedMonth]
    const template = current?.rows[current.rows.length - 1]
    if (!template) {
      addRow()
      return
    }
      ...
      return {
        ...
        rows: [
          ...month.rows,
          {
            ...
            live_days:              template.live_days,
            avg_daily_hours:        template.avg_daily_hours,
            revenue_per_minute_usd: template.revenue_per_minute_usd,
            share_ratio_pct:        template.share_ratio_pct,
          },
        ],
      }
    }))
            ...
            <div className={`flex items-center justify-between gap-4 px-5 py-3.5 ${inputOpen ? 'border-b border-slate-100' : ''}`}>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {inputOpen && canEditActive && (
                  <AddAccountMenu onAddTemplate={addRowFromTemplate} onAddBlank={addRow} />
                )}
                <button
                  type="button"
                  onClick={() => setInputOpen((o) => !o)}
