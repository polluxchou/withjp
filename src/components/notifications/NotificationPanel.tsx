...
export default function NotificationPanel({
  ...
  return (
    <div className="w-[min(360px,calc(100vw-24px))] overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-900 shadow-xl">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
        ...
        <button
          ...
          disabled={!hasUnread}
          className="text-xs font-medium text-indigo-600 hover:text-indigo-700 disabled:text-slate-300 disabled:hover:text-slate-300"
        >
          {t('markAllRead')}
        </button>
      </div>
          ...
          {visibleNotifications.map((notification) => {
            ...
            return (
              <button
                ...
                className={`flex w-full gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-100 ${
                  unread ? 'bg-indigo-50/80' : 'bg-white'
                }`}
              >
                <span className="mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md bg-white ring-1 ring-slate-200">
