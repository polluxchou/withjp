...
- 增删竞品的写权限**收敛到管理员**（`is_admin`）；非管理员为只读，界面隐藏增删控件（服务端仍强制校验）。
...
- `competitors`（竞品清单，唯一键 `unique(platform, handle)`）
- `competitor_snapshots`（每日打点，唯一键 `unique(competitor_id, captured_on)`）

两表均启用 RLS + `authenticated_only`（`for all to authenticated`）策略；写权限在 service 层按 `is_admin` 再收紧，快照写入走 service-role。`platform` 带 `check (platform in ('tiktok'))`，为将来扩平台预留。
...
- `src/lib/competitors/service.ts` — 看板加载 + 清单 CRUD + 快照 upsert（`ServiceResult<T>`）
...
- `PATCH /api/competitors/[id]`（更新备注 / 昵称）
- `DELETE /api/competitors/[id]`（删除竞品及其全部打点）

单元测试（`node:test`）：`metrics.test.ts`、`chart.test.ts`、`assemble.test.ts`。
...
1. 管理员在看板粘贴 TikTok 主页链接 / `@handle` 添加竞品（也可直接由脚本创建）。
2. 用内置浏览器读取该主页指标。
...
- 不采集直播间实时数据（在线人数等瞬时指标）。
- 不采集单条视频 / 作品明细数据。
- 清单增删仅管理员；快照写入只走脚本。
- 更新备注 / 昵称的 `PATCH` API 已具备，但暂未接 UI。
