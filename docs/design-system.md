...
- **Stat / StatBand** `label` `value` `delta` `note` `tone` `onClick?`；数字自动 tabular；`delta.tone` 仅 `success|danger`；负值 `value` 由调用方显式传 `tone="danger"`（`value` 为 `ReactNode` 无法自动判负）；`onClick` 存在时根节点渲染为 `<button>`（键盘可达 + focus ring），常见于列表页 KPI 的「点击筛选/再点清除」交互（支出管理页样板）
